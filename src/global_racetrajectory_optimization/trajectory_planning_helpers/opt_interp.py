...
def opt_interp(reftrack: np.ndarray,
    ...
    #! Everything concerning miniimum curvatuer
    # ------------------------------------------------------------------------------------------------------------------
    # PREPARATIONS -----------------------------------------------------------------------------------------------------
    # ------------------------------------------------------------------------------------------------------------------
    ...
    #! Everythinc concerning minimum distance
    H_2 = np.zeros((no_points, no_points))
    f_2 = np.zeros(no_points)

    for i in range(no_points):
        if i < no_points - 1:
            H_2[i, i] += 2 * (math.pow(normvectors[i, 0], 2) + math.pow(normvectors[i, 1], 2))
            H_2[i, i + 1] = 0.5 * 2 * (-2 * normvectors[i, 0] * normvectors[i + 1, 0]
                                     - 2 * normvectors[i, 1] * normvectors[i + 1, 1])
            H_2[i + 1, i] = H_2[i, i + 1]
            H_2[i + 1, i + 1] = 2 * (math.pow(normvectors[i + 1, 0], 2) + math.pow(normvectors[i + 1, 1], 2))

            f_2[i] += 2 * normvectors[i, 0] * reftrack[i, 0] - 2 * normvectors[i, 0] * reftrack[i + 1, 0] \
                    + 2 * normvectors[i, 1] * reftrack[i, 1] - 2 * normvectors[i, 1] * reftrack[i + 1, 1]
            f_2[i + 1] = -2 * normvectors[i + 1, 0] * reftrack[i, 0] \
            ...
            H_2[i, i] += 2 * (math.pow(normvectors[i, 0], 2) + math.pow(normvectors[i, 1], 2))
            H_2[i, 0] = 0.5 * 2 * (-2 * normvectors[i, 0] * normvectors[0, 0] - 2 * normvectors[i, 1] * normvectors[0, 1])
            H_2[0, i] = H_2[i, 0]
            H_2[0, 0] += 2 * (math.pow(normvectors[0, 0], 2) + math.pow(normvectors[0, 1], 2))

            f_2[i] += 2 * normvectors[i, 0] * reftrack[i, 0] - 2 * normvectors[i, 0] * reftrack[0, 0] \
                    + 2 * normvectors[i, 1] * reftrack[i, 1] - 2 * normvectors[i, 1] * reftrack[0, 1]
            f_2[0] += -2 * normvectors[0, 0] * reftrack[i, 0] - 2 * normvectors[0, 1] * reftrack[i, 1] \
    ...
    alpha_mincurv = quadprog.solve_qp(alpha* H + (1-alpha)* H_2, alpha * -f - (1-alpha) * f_2, -G.T, -h, 0)[0]

    # print runtime into console window
    if print_debug:
        print("Solver runtime opt_interp: " + "{:.3f}".format(time.perf_counter() - t_start) + "s")
...
if __name__ == "__main__":
    ...
    alpha_mincurv, curv_error_max = opt_avg(reftrack=reftrack,
