...
class WorldGenerator(object):
    ...
    def __init__(self, height_map, out_dir, config={"resolution": 0.050000}):
        self.config = config
        self.out_dir = out_dir
        if not os.path.exists(out_dir):
            os.makedirs(out_dir)
        self.map = cv2.imread(height_map, -1).astype(np.float)

        _, self.map = cv2.threshold(self.map, 127, 255, cv2.THRESH_BINARY_INV)
        self.map = self.map.astype(float)
        print(self.map)
        ...
        # self.resolution = yaml_data['resolution'] 
         ...
         #! Play arounf with these parameters
        # self.resolution = 0.050000
        # self.origin = np.array([-self.resolution * self.map.shape[1] // 2 , -self.resolution * self.map.shape[0] // 2])
    ...
    def generate_map(self):
        ...
        with open(os.path.join(self.out_dir, "map.yml"), 'w') as outfile:
            yaml.dump(yaml_file_content, outfile, default_flow_style=False)

        cv2.imwrite(os.path.join(self.out_dir, "map.png"), self.map)
